...
def save_note():
    ...
    reminder_count = d.get("reminderCount", 0)
    ...
    if d.get("id"):
        c.execute("""
            UPDATE notes SET title=%s, keywords=%s, content=%s, updated_at=%s
            WHERE id=%s
        """, (d["title"], d["keywords"], d["content"], now, d["id"]))

        c.execute("""
            UPDATE items
            SET title=%s, preview=%s, updated_at=%s, reminder_count=%s
            WHERE type='note' AND ref_id=%s
        """, (d["title"], d["content"][:60], now, reminder_count, d["id"]))
    ...
    """, (nid, d["title"], d["content"][:60], now, reminder_count))
